...
export default function Stats({ month, monthText }) {
    ...
    const getDataFromLocal = (month) => {
        const data = localStorage.getItem('roxiler' + month);
    ...
    const setDataFromLocal = () => {
        localStorage.setItem('roxiler', JSON.stringify(data));
    }
    ...
    const getData = async () => {
        try {
            setLoading(true);
            // const res = await axios.get(`http://localhost:8080/api/combined-data?month=${month}`, {
